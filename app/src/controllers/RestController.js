...
export default class RESTController extends Controller {
  ...
  processAnyErrors(request, response) {
    ...
    if (response.status === 0) {
      this.apiModel.noInternet = true;

      this.apiModel.errors = this.apiModel.errors.concat([{
        request,
        response
      }]);
    }
  ...
  request(props) {
    return new Promise((resolve, reject) => {
      ...
      this.apiModel.noInternet = false;
      ...
      if (props.cacheRequest && props.type === 'GET' && (cacheItem = this.apiModel.getCacheFor(url))) {
        ...
        let handleError = () => {
          ...
          if (!props.$ringaEvent.detail.apiErrorHandler || props.$ringaEvent.detail.apiErrorHandler(response)) {
        ...
        let handleResult = () => {
          ...
          if (props.cacheRequest) {
            this.apiModel.finishCacheRequest(url, parsedResponse);
          }
        ...
        if (props.cacheRequest) {
          this.apiModel.startCacheRequest(url, props.cacheRequest.timeout);
        }
